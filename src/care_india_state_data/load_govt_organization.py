...
def get_state(state_name):
    if state_name not in state_cache:
        try:
            state_cache[state_name] = Organization.objects.get(name=state_name, root_org=None)
...
def get_district(state_name, district_name):
    ...
    if district_name not in districts_cache[state_name]:
        try:
            districts_cache[state_name][district_name] = Organization.objects.get(name=district_name, parent=state)
        except Organization.DoesNotExist:
            logger.error(
...
def get_local_body(state_name, district_name, local_body_name):
    ...
    if not district:
        return None
    try:
        return Organization.objects.filter(name=local_body_name, parent=district).first()
    except Organization.DoesNotExist:
        logger.error(
...
class GovtOrgLoader:
    """Load government organization data from JSON files."""

    def __init__(self):
        self.migration_id = int(datetime.now(tz=UTC).timestamp() * 1000)
        self.country = "India"
        self.state = None
        self.load_districts = False
        self.load_local_bodies = False
        self.load_wards = False
        self.json_file_path: Path = Path(__file__).parent / "data/india/states-and-districts.json"
    ...
    def load_state_and_district_data(self):
        ...
        for item in data:
            ...
            for d in districts:
                ...
                if not created:
                    logger.debug("District already exists: %s (%s)", district.name, district.id)
                else:
                ...
                local_body_objs.append(
                    Organization(
                        ...
                        metadata={
                            ...
                            "govt_org_id": local_body.get("lsg_code", local_body.get("localbody_code")),
                        },
                        meta={
                            "migration_id": self.migration_id,
                        },
            ...
            for f in sorted((state_dir / "lsg").glob("*.json")):
                with f.open() as data_f:
                    ...
                    if data.get("district") is not None:
                        local_body = get_local_body(data["state"], data["district"], get_local_body_name(data))
                        if not local_body:
                            continue
    ...
    def run(self, state="all", load_districts=False, load_local_bodies=False, load_wards=False, verbosity=1):
        ...
        self.state = state
        self.load_districts = load_districts
        self.load_local_bodies = load_local_bodies
        self.load_wards = load_wards
        ...
        with transaction.atomic():
            ...
            root_dir: Path = Path(__file__).parent / "data/india"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Load government organization data from JSON files into Django database."
    )
    parser.add_argument(
        "--state",
        default="all",
    ...
    loader = GovtOrgLoader()
    loader.run(
        state=args.state,
        load_districts=args.load_districts,
        load_local_bodies=args.load_local_bodies,
        load_wards=args.load_wards,
